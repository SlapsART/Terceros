...
function EventoDot({ tipo }: EventoDotProps) {
...
function EventoRow({ evento, isLast, expanded, onToggle }: EventoRowProps) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1.5, px: 1, alignItems: 'flex-start', width: '100%' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: 16, alignSelf: 'stretch' }}>
        ...
        {!isLast && (
          <Box sx={{ flex: '1 0 0', width: '2px', bgcolor: 'divider', minHeight: '4px' }} />
        )}
      </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, pb: 1.5 }}>
            ...
            {evento.motivo && (
              <Box>
                ...
                <Typography
                  variant="body2"
                  sx={{ fontSize: '0.8125rem', lineHeight: '16px', fontStyle: 'italic' }}
                >
                  {evento.motivo}
                </Typography>
              </Box>
...
export function HistorialDrawer({ open, eventos, onClose }: HistorialDrawerProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const result = q
      ? eventos.filter((e) => e.titulo.toLowerCase().includes(q))
      : [...eventos];
  ...
  const toggleExpandAll = () => {
    if (allExpanded) {
      ...
      setExpandedIds(new Set(allExpandable.map((e) => e.id)));
    }
  };
