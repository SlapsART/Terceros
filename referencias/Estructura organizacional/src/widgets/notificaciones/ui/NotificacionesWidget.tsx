...
import sincoLogoUrl from '@/assets/sinco-logo.svg';
...
/* Entrada nueva notif — spring con overshoot y follow-through */
...
/* Entrada cards en panel — pop desde arriba */
const enterPanelCard = keyframes`
  from { opacity: 0; transform: translateY(-6px) scale(0.97); }
  to   { opacity: 1; transform: translateY(0)    scale(1);    }
`;

/* Pill "Todo revisado" — misma entrada spring que enterSlide, hold, salida */
...
/* Salida individual (X) — anticipación + lanzamiento */
const exitCard = keyframes`
  0%   { opacity: 1; transform: translateX(0)    scale(1);    }
  10%  { opacity: 1; transform: translateX(-6px) scale(1.01); }
  100% { opacity: 0; transform: translateX(80px) scale(0.87); }
`;

/* Salida cascada (Borrar todo / Borrar grupo) */
const exitAll = keyframes`
  0%   { opacity: 1; transform: translateX(0)    scale(1);     }
  12%  { opacity: 1; transform: translateX(-5px) scale(1.005); }
  100% { opacity: 0; transform: translateX(64px) scale(0.9);   }
`;

/* Salida panel — blur de velocidad */
const exitPanel = keyframes`
  0%   { opacity: 1;   transform: translateY(0)     scale(1);    filter: blur(0px); }
  15%  { opacity: 0.9; transform: translateY(2px)   scale(1.01); filter: blur(0px); }
  100% { opacity: 0;   transform: translateY(-16px) scale(0.93); filter: blur(3px); }
`;
  ...
  return (
    <Box
      ...
      sx={{
        ...
        width: '100%',
        animation: isExiting
          ? `${exitAll} 0.4s ${index * 0.05}s ${EASE_IN} both`
          : completing
            ? `${exitCard} 0.42s ${EASE_IN} forwards`
            : (inPanel && !fresh)
          ...
          <Box
            sx={{
              ...
              animation: clearingAll
                ? `${exitPanel} 0.36s ${EASE_IN} forwards`
